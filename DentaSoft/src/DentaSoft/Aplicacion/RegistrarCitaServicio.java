...
public class RegistrarCitaServicio {
    ...
    public HorarioAtencion buscarHorario(String idhorario, LocalDate fecha, int idDentista) throws Exception {
        accesoDatosJDBC.abrirConexion();
        HorarioAtencion horario = horarioPostgreSQL.buscar(idhorario, idDentista);
...
//        horarioPostgreSQL.Mostrar(modelo);
    ...
    public void guardarCita(Cita cita) throws Exception {
        ...
        if (!citaPostgreSQL.verificarHorarioDisponible(cita.getHorarioatencion(), cita.getFecha())) {
            throw new Exception("EL TURNO ESCOGIDO NO ESTA DISPONIBLE");
        }
    ...
    public List<HorarioAtencion> listasHorario(int documentoDentista, LocalDate fecha) throws Exception {
        accesoDatosJDBC.abrirConexion();
        List<HorarioAtencion> turnos = horarioPostgreSQL.mostrarHorarios(documentoDentista);

        for (HorarioAtencion turno : turnos) {
            turno.setEstado("LIBRE");
            if (fecha != null) {
                boolean libre = citaPostgreSQL.verificarHorarioDisponible(turno, fecha);
                if (!libre) {
